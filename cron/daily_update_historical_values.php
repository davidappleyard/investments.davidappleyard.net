...
function prepend_log_block($log_file, $content) {
    try {
        ...
        file_put_contents($log_file, $content . ($existing_content ? "\n\n\n" . $existing_content : ""));
    } catch (Exception $e) {
        echo "[ERROR] Failed to write to log file {$log_file}: " . $e->getMessage() . "\n";
    }
}
...
log_and_echo("=== Daily Historical Account Values Update ===");
log_and_echo("Target date: {$targetDate}");
log_and_echo("Started at: " . date('Y-m-d H:i:s'));
log_and_echo("");
...
try {
    ...
    log_and_echo("Processing " . count($accounts) . " account combinations for date {$targetDate}...");
    log_and_echo("");
    ...
    foreach ($accounts as [$client, $account]) {
        ...
        if ($stmt->fetch()) {
            log_and_echo("  Skipping {$client} {$account} - record already exists for {$targetDate}");
            $skippedRecords++;
            continue;
        }
        ...
        log_and_echo("  Processed {$client} {$account}: Holdings £" . number_format($holdingsValue, 2) . 
    ...
    log_and_echo("");
    log_and_echo("=== Daily Update Complete ===");
    log_and_echo("Target date: {$targetDate}");
    log_and_echo("Processed records: {$processedRecords}");
    log_and_echo("Skipped records: {$skippedRecords}");
    log_and_echo("Total time: {$elapsed} seconds");
    log_and_echo("Completed at: " . date('Y-m-d H:i:s'));
    
    if ($processedRecords > 0) {
        log_and_echo("");
        log_and_echo("Successfully updated historical values for {$targetDate}.");
    } else {
        log_and_echo("");
        log_and_echo("No new records were created (all records already existed).");
    }
    
    // Write all collected log output to file
    if (!empty($log_output)) {
        $log_content = implode("\n", $log_output);
        prepend_log_block("../logs/daily_update_historical_values.log", $log_content);
    }
