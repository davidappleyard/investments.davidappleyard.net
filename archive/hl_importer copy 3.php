...
function calculate_cash_for_account($pdo, $client, $account): float {
    ...
    $runningTotal = 0.0;
    $cashAffectingTypes = ['Deposit', 'Interest', 'Sell', 'Dividend', 'Dividend Transfer', 'Buy', 'Withdrawal', 'Fee'];
    ...
    foreach ($transactions as $t) {
        ...
        if (in_array($type, ['Deposit', 'Interest', 'Sell', 'Dividend', 'Dividend Transfer'])) {
...
function calculate_historical_account_balance(PDO $pdo, string $client, string $account, string $date): array {
    ...
    $stmt = $pdo->prepare("
        SELECT 
            SUM(CASE 
                WHEN type IN ('Deposit', 'Interest', 'Sell', 'Dividend', 'Dividend Transfer') THEN value_gbp
...
<body>
  ...
  <div style="margin-top:0.9rem; display:grid; gap:0.9rem; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); align-items:start;">
    <div class="card">
